#
# Inspired (a lot) by https://github.com/nicolewhite/neo4j-jupyter
#
...
def vis_network(graph_name, nodes, edges, physics=False,configure=False):
    html = """
    <html>
    <head>
    ...
    file_name_string = slugify(graph_name)
    filename = "figure/graph-{}.html".format(file_name_string)
    html = html.format(id=unique_id, filename=filename, nodes=json.dumps(nodes), edges=json.dumps(edges), physics=json.dumps(physics),configure=json.dumps(configure))
...
def draw(graph_name, graph, options, physics=False, limit=100):
    ...
    return vis_network(graph_name, nodes, edges, physics=physics)
